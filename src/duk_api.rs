...
// duk_push_c_function
pub fn push_c_function(ctx: *mut duk_context, func: extern fn(*mut duk_context) -> i32, nargs: c_int) -> c_int {
    unsafe {
...
pub fn push_lstring<T: Into<Vec<u8>>>(ctx: *mut duk_context, string: T) -> *const c_char {
    ...
    unsafe {
        duk_push_lstring(ctx, cstring_string.as_ptr(), cstring_string.into_bytes().len() as i32)
    }
}
